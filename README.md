...
2. **打开管理后台**：浏览器访问 `http://<服务器IP>:8080/`，登录账户 `admin` / `123456`
...
| 端口 | 用途 |
|------|------|
| `8080`（默认） | 前端页面（`/api` 已自动反代到后端）——日常访问主入口 |
| `6080` | noVNC **网页版** VNC：浏览器直接访问 `http://IP:6080/` 即可看到抖音窗口，无需安装 VNC 客户端 |
| `5900` | VNC（任意 VNC 客户端连接，用于扫码后**手动二次验证**，如滑块） |
| `9844` | 后端 FastAPI 直连（调试 / 接口直调 / 健康检查） |

> 服务器若禁止 80 端口，前端入口默认用 **8080**；可通过环境变量 `WEB_PORT` 改任意端口（如 `WEB_PORT=3000 docker compose up -d`）。
> 四个端口都已对外映射。**宿主机防火墙/云安全组需放行** `8080`、`6080`、`9844`（如用 VNC 客户端还需 `5900`）。
...
推荐直接用网页版：浏览器打开 `http://<服务器IP>:6080/`；也可用任意 VNC 客户端连接 `:5900`。

> 适用场景：扫码登录后抖音会弹出二次验证（如滑块），原项目无法自动化这一步，
> 需要在 VNC 窗口里手动完成（抖音窗口为标准横版 1280x720，可拖动；按钮均可直接操作）。
