...
### 登录向导（推荐）
打开左侧菜单 **「登录向导」**，扫码 / 手机验证码 / 二次验证在一个页面里完成，**不用再手动开 VNC**：
- **内置实时画面**：后端通过 Chromium DevTools 协议把浏览器画面（JPEG 帧）经 WebSocket 推到网页，鼠标 / 键盘 / **触摸**操作实时回传
  （手机浏览器可直接点、可直接触摸操作，支持「放大」按钮与双击画面全屏）。
- **一键获取 / 刷新登录二维码**，扫码后出现二次验证（**接收手机验证码 / 扫码人脸识别**）时，直接在画面里完成即可。
- **兜底模式**：万一画面不通，可切「截图模式」（每秒一帧截图 + 点击转发），或点右上角「打开 noVNC」用传统方式。
- 在向导里操作期间，定时任务会自动让路，避免打断人工验证。
...
### 首页看板
- 浏览器 / 登录状态三态显示（待初始化 → 待登录抖音 → 系统运行正常）
- 好友数量 / 定时任务数量统计（未登录时也能看到已保存的任务）
- 快速操作：初始化浏览器、重新初始化浏览器、刷新好友列表、管理定时任务
- 系统运行信息（版本、在线时长、调度器、noVNC 入口）

### 界面与主题
- **暗黑 / 明亮主题**：顶栏图标一键切换，也可在 **设置 → 界面外观 → 暗黑模式** 开关里切换；
  选择会保存在浏览器（`localStorage`），下次打开自动生效，且首屏不会闪白。

### 界面预览（亮色 / 暗色）

首页看板支持**亮色与暗色模式**，顶栏图标一键切换（设置页也有开关，且会自动记住你的选择）：
...
| `5900` | VNC（任意 VNC 客户端连接，用于手动完成二次验证）——默认也不映射 |

> 端口全部可自定义：前端用 `WEB_PORT`、noVNC 用 `NOVNC_PORT`、后端用 `API_PORT`。
> 改完端口映射后，把同名环境变量也设成一样的值，首页显示的「前端端口 / noVNC 端口」会自动跟随，
> **不需要重新构建镜像**（前端端口本身取自浏览器地址栏）；后端未映射时首页会自动隐藏「API 地址」一行。
...
**只用登录向导、完全不用 VNC**：把 `VNC_ENABLED` 设为 `0` 即可关闭 x11vnc / noVNC（此时可删掉 compose 里的 `14321` 端口映射）。
...
Docker 部署相关的几个变量（写在 `docker-compose.yml` 里）：

| 变量 | 说明 | 默认值 |
|------|------|--------|
| `WEB_PORT` | 前端对外端口 | `14320` |
| `NOVNC_PORT` | noVNC 对外端口（同时用于首页展示） | `14321` |
| `API_PORT` | 后端对外端口；**不设置即视为未映射**，首页会隐藏「API 地址」 | 空 |
| `VNC_PASSWORD` | VNC / noVNC 密码（**建议修改**） | `123456` |
| `VNC_ENABLED` | 是否启动 x11vnc / noVNC；设 `0` 可完全关闭，仅用登录向导的画面串流 | `1` |

> Docker 部署无需配置上面的后端变量（镜像已内置默认值）；仅本地部署按需设置。
...
| `/login` | 登录页 | 管理员账户登录 |
| `/home` | 首页 | 状态看板、快速操作 |
| `/wizard` | 登录向导 | 扫码 / 手机验证码 / 二次验证一站式完成（含实时画面） |
| `/friends` | 好友列表 | 查看好友、发送消息 |
| `/tasks` | 定时任务 | 添加/修改/删除任务 |
| `/settings` | 设置 | 界面外观（暗黑模式）、登录管理、密码修改、调试功能 |
